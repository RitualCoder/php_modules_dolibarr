 ...
 * Copyright (C) 2020		Enzo Cornaggia		<enzo.cornaggia@icloud.com>
 *
...
/**
 *	Class to build sending documents with model Shipment
 */
class pdf_shipment extends ModelePdfExpedition
{
	/**
		...
		if ($conf->expedition->dir_output) {
			...
			if (file_exists($dir)) {
				...
				if ($pagenb == 1) {
					...
					$this->_tableau($pdf, $tab_top_newpage, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforfooter - 30, 0, $outputlangs, 1, 0);
					$bottomlasttab = $this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
				}
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0)
	{
		...
		// background color table title (230,230,230)
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if (!empty($conf->$origin->enabled)) {     // commonly $origin='commande'
			...
			if ($result >= 0) {
				...
				$Yoff = $Yoff + 8;
				$pdf->SetXY($this->page_largeur - $this->marge_droite - $w, $Yoff - 6);
	...
	protected function _signature_area(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$pdf->SetFont('', '', $default_font_size - 2);
		$pdf->MultiCell($largcol, $tab_hl, $outputlangs->transnoentities("ShipmentCustomerSignature"), 0, 'L', 1);
