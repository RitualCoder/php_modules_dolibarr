 ...
 * Copyright (C) 2022	   Enzo Cornaggia		<enzo.cornaggia@icloud.com>
 *
...
/**
 *	Class to generate PDF proposal Propale
 */
class pdf_propale extends ModelePDFPropales
{
	/**
		...
		if ($conf->propal->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					}elseif(substr($object->lines[$i]->description, 0, 5) == '<div>' && empty(pdf_getlineqty($object, $i, $outputlangs, $hidedetails))){
						$isComment = 1;
					}else{
						$isTitre = 0;
						$isComment = 0;
					...
					// Unit
					if (!empty($conf->global->PRODUCT_USE_UNITS) && empty($isTitre) && empty($isComment)) {
					...
					// Add line
					if (!empty($conf->global->MAIN_PDF_DASH_BETWEEN_LINES) && $i < ($nblines - 1) && !empty($isTitre) && $i>0) {
						$pdf->setPage($pageposafter);
					...
					while ($pagenb < $pageposafter) {
						$pdf->setPage($pagenb);
						if ($pagenb == 1) {
							$this->_tableau($pdf, $tab_top, $this->page_hauteur - $tab_top - $heightforfooter, 0, $outputlangs, 0, 1, $object->multicurrency_code, $pagenb);
						} else {
							$this->_tableau($pdf, $tab_top_newpage, $this->page_hauteur - $tab_top_newpage - $heightforfooter, 0, $outputlangs, 0, 1, $object->multicurrency_code, $pagenb);
						}
				...
				// Show square
				if ($pagenb == 1) {
					$this->_tableau($pdf, $tab_top, $this->page_hauteur - $tab_top - $heightforinfotot - $heightforfreetext - $heightforsignature - $heightforfooter, 0, $outputlangs, 0, 0, $object->multicurrency_code, $pagenb);
					$bottomlasttab = $this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforsignature - $heightforfooter + 1;
				} else {
					$this->_tableau($pdf, $tab_top_newpage, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforsignature - $heightforfooter, 0, $outputlangs, 0, 0, $object->multicurrency_code, $pagenb);
					$bottomlasttab = $this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforsignature - $heightforfooter + 1;
				}
	 ...
	 *   @return	void
	 */
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '', $pagenb)
	{
		global $conf;
		...
		if (empty($hidetop)) {
			if ($pagenb == 1){
				...
				$titre = $outputlangs->transnoentities("AmountInCurrency", $outputlangs->transnoentitiesnoconv("Currency".$currency));
				$pdf->SetXY($this->page_largeur - $this->marge_droite - ($pdf->GetStringWidth($titre) + 3), $tab_top - 9);
				$pdf->MultiCell(($pdf->GetStringWidth($titre) + 3), 2, $titre);

				//$conf->global->MAIN_PDF_TITLE_BACKGROUND_COLOR='230,230,230';
				if (!empty($conf->global->MAIN_PDF_TITLE_BACKGROUND_COLOR)) {
					$pdf->Rect($this->marge_gauche, $tab_top - 5, $this->page_largeur - $this->marge_droite - $this->marge_gauche, 5, 'F', null, explode(',', $conf->global->MAIN_PDF_TITLE_BACKGROUND_COLOR));
				}
			}
		}
		...
		if ($pagenb == 1){
			...
			$this->printRect($pdf, $this->marge_gauche, $tab_top - 5, $this->page_largeur - $this->marge_gauche - $this->marge_droite, $tab_height + 5, $hidetop, $hidebottom); // Rect takes a length in 3rd parameter and 4th parameter
		}
		...
		if (empty($hidetop)) {
			if ($pagenb == 1){
				...
				$pdf->line($this->marge_gauche, $tab_top, $this->page_largeur - $this->marge_droite, $tab_top); // line takes a position y in 2nd parameter and 4th parameter

				$pdf->SetXY($this->posxdesc - 1, $tab_top - 4);
				$pdf->MultiCell(108, 2, $outputlangs->transnoentities("Designation"), '', 'L');
			}
		}
		...
		if ($pagenb == 1){
			...
			$pdf->line($this->posxup - 1, $tab_top - 5, $this->posxup - 1, $tab_top + $tab_height);
		}
		...
		if (empty($hidetop)) {
			if ($pagenb == 1){
				...
				$pdf->SetXY($this->posxup - 1, $tab_top - 4);
			}
		...
		if ($pagenb == 1){
			...
			$pdf->line($this->posxqty - 1, $tab_top - 5, $this->posxqty - 1, $tab_top + $tab_height);
		}
		...
		if (empty($hidetop)) {
			if ($pagenb == 1){
				...
				$pdf->SetXY($this->posxqty - 1, $tab_top - 4);
			}
		...
		if (!empty($conf->global->PRODUCT_USE_UNITS)) {
			if ($pagenb == 1){
				...
				$pdf->line($this->posxunit - 1, $tab_top - 5, $this->posxunit - 1, $tab_top + $tab_height);
			}
			if (empty($hidetop)) {
		...
		if ($pagenb == 1){
			...
			$pdf->line($this->posxdiscount - 1, $tab_top - 5, $this->posxdiscount - 1, $tab_top + $tab_height);
		}
		if (empty($hidetop)) {
			if ($this->atleastonediscount) {
				if ($pagenb == 1){
					...
					$pdf->SetXY($this->posxdiscount - 1, $tab_top - 4);
				}
		...
		if ($this->atleastonediscount) {
			if ($pagenb == 1){
				...
				$pdf->line($this->postotalht, $tab_top - 5, $this->postotalht, $tab_top + $tab_height);
			}
		}
		if (empty($hidetop)) {
			if ($pagenb == 1){
				...
				$pdf->SetXY($this->postotalht - 1, $tab_top - 4);
			}
		...
		if (empty($conf->global->PDF_DISABLE_MYCOMPANY_LOGO)) {
			if ($this->emetteur->logo) {
				...
				if (is_readable($logo)) {
					...
					$pdf->Image($logo, $this->marge_gauche, $posy - 2, 0, $height + ($height*0.25));	// width=0 (auto)
				} else {
					$pdf->SetTextColor(200, 0, 0);
					$pdf->SetFont('', 'B', $default_font_size - 2);
		...
		*/
		...
		if ($showaddress) {
			...
			if (empty($conf->global->MAIN_PDF_NO_RECIPENT_FRAME)) {
				...
				$pdf->Rect($posx, $posy - 4, $widthrecbox, $hautcadre);
			}
	...
	protected function _signature_area(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$pdf->SetXY($posx, $tab_top + $tab_hl + 1);
		$pdf->MultiCell($largcol, $height * 3, '', 1, 'R');
		...
		$pdf->SetXY($posx, $tab_top + 0);
		$pdf->SetFont('', '', $default_font_size - 4);
		$pdf->MultiCell($largcol, $tab_hl, "Pénalités de retard (taux annuel) : Taux Refi BCE + 10%\nEscompte pour paiement anticipé (taux mensuel) : 0%\nRESERVE DE PROPRIETE: Nous nous réservons la propriété des marchandises jusqu'au complet paiement du prux par l'acheteur.\nNotre droit de revendication porte aussi bien sur les marchandises que sur leur prix si elles ont dejà été revendues (Loi du 12 mars 1980).", 1, 'L', 1);
