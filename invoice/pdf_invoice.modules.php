...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_invoice.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from invoice model
 */
...
/**
 *	Class to generate the customer invoice PDF with template Invoice
 */
class pdf_invoice extends ModelePDFFactures
{
	 /**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->dir_output) {
			...
			if (file_exists($dir)) {
				...
				// Show square
				if ($pagenb == 1) {
					$this->_tableau($pdf, $tab_top, $this->page_hauteur - $tab_top - $heightforinfotot - $heightforfreetext - $heightforfooter, 0, $outputlangs, 0, 0, $object->multicurrency_code, $pagenb);
					$bottomlasttab = $this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
				} else {
					$this->_tableau($pdf, $tab_top_newpage, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforfooter, 0, $outputlangs, 0, 0, $object->multicurrency_code, $pagenb);
					$bottomlasttab = $this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
				}
		...
		if ($this->situationinvoice) {
			...
			if (empty($hidetop)) {
				if ($pagenb == 1){
					...
					$pdf->SetXY($this->posxprogress, $tab_top - 4);
				}
